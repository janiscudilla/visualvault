...
function App() {
  ...
  function changeView(nextView) {
    setView(nextView);
    if (nextView === 'people') setRearranging(false);
  }
      ...
      <main>
        ...
        <section className="controls" aria-label="Find and arrange photos">
          ...
          <div className="control-row">
            ...
            <label className="sort-control"><ArrowDownAZ size={15} /><select value={sort} onChange={(event) => { const nextSort = event.target.value; setSort(nextSort); setRearranging(nextSort === 'manual' && view !== 'people'); }} aria-label="Sort photos"><option value="manual">Custom arrangement</option><option value="newest">Newest added</option><option value="oldest">Oldest added</option><option value="person">Person A–Z</option><option value="group">Group A–Z</option><option value="most">Most saved person</option></select></label>
            {isOwner && view !== 'people' && <button className={`arrange-button ${rearranging ? 'active' : ''}`} type="button" onClick={toggleRearranging}><LayoutGrid size={15} /> {rearranging ? 'Done arranging' : 'Arrange photos'}</button>}
