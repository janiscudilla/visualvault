...
function App() {
  ...
  return (
    <div className="app-shell">
      ...
      {editorOpen && isOwner && <AddEditModal photo={editorPhoto} user={user} people={people} groups={groups} onClose={() => setEditorOpen(false)} onSaved={(warning, count = 1) => { setEditorOpen(false); setToast(warning ? 'Photo saved. The original site blocked copying, so this one still uses its URL.' : count > 1 ? `${count} photos added to your folder.` : editorPhoto ? 'Photo updated.' : 'Photo added to your folder.'); }} onDeleted={() => { setEditorOpen(false); setToast('Photo removed.'); }} />}
