...
function AlbumCard({ photo, onOpen, canReorder, dragging, onDragStart, onDragEnter, onDragOver, onDrop, onDragEnd }) {
  return (
    <button
      ...
      onDragEnter={(event) => onDragEnter?.(event, photo.id)}
...
function AlbumView({ photos, onOpen, density, canReorder, onReorder }) {
  ...
  function moveOverPhoto(event, targetId) {
    allowDrop(event);
    const sourceId = draggedRef.current || event.dataTransfer.getData('text/plain');
        ...
        <section className="binder-page" key={`page-${page[0]?.id || pageIndex}`} aria-label={`Album page ${pageIndex + 1}`}>
          ...
          <div className="pocket-grid">
            {Array.from({ length: pageSize }, (_, cellIndex) => {
              ...
              return photo
                ? <AlbumCard key={photo.id} photo={photo} onOpen={onOpen} canReorder={canReorder} dragging={draggedId === photo.id} onDragStart={startDrag} onDragEnter={moveOverPhoto} onDragOver={allowDrop} onDrop={finishDrag} onDragEnd={finishDrag} />
